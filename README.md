...
│   │   ├── dataResolvers.ts          # 数据字段解析
│   │   ├── historyBuilder.ts         # 历史数据构建
│   │   └── todayStatsResolver.ts     # 今日统计解析
│   ├── styles/                       # 全局样式和设计色板
│   ├── utils/                        # 工具函数和演示数据
│   └── types.ts                      # 类型定义
├── astro.config.mjs
...
访问终端输出的本地地址查看应用，默认是 `http://localhost:4321`。如果 `4321` 被占用，开发服务器会自动顺延到下一个可用端口。
...
| `AI_MODEL`          |  是  | AI 模型名称          |
| `OPENROUTER_API_KEY` | 否 | `openrouter` 服务 API Key |
| `DEEPSEEK_API_KEY`  |  否  | `deepseek` 服务 API Key |
| `SILICONFLOW_API_KEY` | 否 | `siliconflow` 服务 API Key |
| `MOONSHOT_API_KEY`  |  否  | `moonshot` 服务 API Key |
| `ZENMUX_API_KEY`    |  否  | `zenmux` 服务 API Key |
| `CUSTOM_API_KEY`    |  否  | `custom` 服务 API Key |
| `AI_BASE_URL`       |  否  | 仅 `custom` 模式需要 |
| `API_SECRET_TOKEN`  |  是  | API 访问控制令牌     |

> **注意**：根据 `AI_PROVIDER` 选择对应的 API Key 环境变量即可，不需要同时配置所有 AI 服务商。
...
项目会根据部署环境自动选择 adapter：检测到 `NETLIFY` 环境变量时使用 Netlify adapter，否则默认使用 Vercel adapter。当前配置没有写死 `site` 地址，因此同一套代码可以部署到 Vercel 或 Netlify。
